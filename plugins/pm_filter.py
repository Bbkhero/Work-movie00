...
async def group_search(client, message):
    ...
    if message.chat.id == SUPPORT_GROUP :
                ...
                if total != 0:
                    ...
                    msg = await message.reply_text(script.SUPPORT_GRP_MOVIE_TEXT.format(message.from_user.mention() , total) ,             reply_markup=InlineKeyboardMarkup([
                        [ InlineKeyboardButton('Get Files From Here' , url=link)]
                        ]))
    ...
    if settings["auto_filter"]:
        if not user_id:
            await message.reply("<b>Im Not Working For Anonymous Admins!</b>")
            return
        ...
        if 'hindi' in message.text.lower() or 'tamil' in message.text.lower() or 'telugu' in message.text.lower() or 'malayalam' in message.text.lower() or 'kannada' in message.text.lower() or 'english' in message.text.lower() or 'gujarati' in message.text.lower(): 
            ...
            return await message.reply("<b> Sending Link Is Not Allowed Here</b>")
        ...
        k=await message.reply_text('<b>Auto Filter Mode is Off...</b>')
...
async def next_page(bot, query):
    ...
    del_msg = f"\n\n<b>This Message Will Be deleted After<code>{get_readable_time(DELETE_TIME)}</code> To Avoid ©️ Copyright Infringement </b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
    ...
    btn.insert(0,[
        InlineKeyboardButton("Choose Language ", callback_data=f"languages#{key}#{offset}#{req}"),
        ])
    btn.insert(1, [
    ...
    btn.insert(3,[
        InlineKeyboardButton("Send All", callback_data=batch_link),
        ])
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    await query.message.edit_text("<b> Which Season Do You Want , Choose From Here↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def season_search(client: Client, query: CallbackQuery):
    ...
    if not files:
        ...
        if not files:
            await query.answer(f"SORRY {season.title()} NOT FOUND FOR {search}", show_alert=1)
            return
    ...
    settings = await get_settings(query.message.chat.id)
    temp.CHAT[query.from_user.id] = query.message.chat.id
    del_msg = f"\n\n<b>⚠️ THIS MESSAGE WILL BE AUTO DELETE AFTER <code>{get_readable_time(DELETE_TIME)}</code> TO ABOID COPYRIGHT ISSUESU</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
        for file_num, file in enumerate(files, start=offset+1):
    ...
    btn.append([
        InlineKeyboardButton(text="⪻ Back To Main Page", callback_data=f"next_{req}_{key}_{orginal_offset}"),])
    await query.message.edit_text(cap + links + del_msg, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="⪻ Back To Main Page ", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text("<b>Which Year Do You Want, Choose From Here↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def year_search(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer(f"Sorry {year.title()} Not Found For{search}", show_alert=1)
        return
    ...
    settings = await get_settings(query.message.chat.id)
    temp.CHAT[query.from_user.id] = query.message.chat.id
    del_msg = f"\n\n<b>This Missage Will Be Auto Deleted After<code>{get_readable_time(DELETE_TIME)}</code> To Avoid Copyright Issue</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
        for file_num, file in enumerate(files, start=offset+1):
...
async def quality_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="⪻Back To Main Page", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text("<b>In Which Quality Do You Want, Choose From Here↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def quality_search(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer(f"Sorry Quality {qul.title()} Not Founde For {search}", show_alert=1)
        return
    ...
    settings = await get_settings(query.message.chat.id)
    temp.CHAT[query.from_user.id] = query.message.chat.id
    del_msg = f"\n\n<b>This Message Will Be Auto Delete After <code>{get_readable_time(DELETE_TIME)}</code> To Avoid Copyright Issues</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
        for file_num, file in enumerate(files, start=offset+1):
    ...
    btn.insert(2, [
        InlineKeyboardButton("♻️ Send All", callback_data=batch_link),
    ])
    if n_offset== '':
        btn.append(
            [InlineKeyboardButton(text="No More Pages ", callback_data="buttons")]
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="⪻ Back To Main Page", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text("<b>In Which Language You Want , Choose From Here↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    if not files:
        ...
        if not files:
            return await query.answer(f"Sorry , Language {lang.title()} Not Found For {search}", show_alert=1)
    ...
    temp.CHAT[query.from_user.id] = query.message.chat.id
    del_msg = f"\n\n<b>This Message Will Be Deleted After<code>{get_readable_time(DELETE_TIME)}</code> To Avoid Copyright Issue</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
        for file_num, file in enumerate(files, start=offset+1):
    ...
    btn.insert(0,[
        InlineKeyboardButton("Choose Quality", callback_data=f"qualities#{key}#{offset}#{req}"),
        ])
    btn.insert(1, [
        InlineKeyboardButton("Year", callback_data=f"years#{key}#{offset}#{req}"),
        InlineKeyboardButton("Season", callback_data=f"seasons#{key}#{offset}#{req}")
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if int(user) != 0 and query.from_user.id != int(user):
            return await query.answer(script.ALRT_TXT, show_alert=True)
        await query.answer("Thank For Close")
        ...
        return await query.message.edit(script.REF_POINT.format(newPoint) , reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton('Get Your Refferal Link', callback_data=f'free_premium#{query.from_user.id}')],   
                [InlineKeyboardButton('⋞ Home', callback_data='start')],]))
        
    elif query.data == "premium":
        userid = query.from_user.id
        await query.message.edit(script.PREMIUM_TEXT , reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton('Low Price Plan ', callback_data='plans')],
        [InlineKeyboardButton('⋞ Home', callback_data='start')]
        ]))
        ...
        if AUTH_CHANNEL and not await is_req_subscribed(client, query):
            await query.answer("I Like Your Smarteness But Dont Be Oversmart\nFirt Join Our Update Channel", show_alert=True)
            return         
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('No.Such File Existed')
        files = files_[0]
        btn = [[
            InlineKeyboardButton('Get Your File', url=f'https://t.me/{temp.U_NAME}?start=file_{chat_id}_{file_id}')
        ]]
        reply_markup = InlineKeyboardMarkup(btn)
        return await query.message.edit(text=f'<b>Thanks For Joining Our Chanenl\nGet Your File : {files.file_name[:20]}.. By Clicking The Button Below\n\n Happy To See You</b>',reply_markup=reply_markup)
        ...
        await query.answer("This Is Page Button")

    elif query.data.startswith("lang_art"):
        _, lang = query.data.split("#")
        await query.answer(f"Your Selected {lang.title()} Language", show_alert=True)
        ...
        [
            InlineKeyboardButton('Get Premium Free', callback_data=f'free_premium#{query.from_user.id}')
        ],
        [
            InlineKeyboardButton('Your Points ', callback_data=f'point#{query.from_user.id}'),
            InlineKeyboardButton('About', callback_data=f'about')
        ],
        [
            InlineKeyboardButton('Earn Money From Bot', callback_data='earn')
        ]]    
        ...
        buttons = [[
            InlineKeyboardButton('T-Graph', callback_data='telegraph'),
            InlineKeyboardButton('Font', callback_data='font')    
        ],
        [
          InlineKeyboardButton('Fsun', callback_data='fsub'),
          InlineKeyboardButton('Ai TTS', callback_data='tts')],[
        InlineKeyboardButton('Admin Cmd', callback_data='admincmd'),
	    InlineKeyboardButton('⋞ Home', callback_data='start')
        ]] 
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(                     
        ...
        buttons = [[
            InlineKeyboardButton('⋞ Home', callback_data='start'),
            InlineKeyboardButton('Support', user_id = ADMINS[0] ),
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.message.edit_text(f"<b>Fetching File For Your Query {keyword} On Db...\n\nPleas Wait...</b>")
        files, total = await get_bad_files(keyword)
        await query.message.edit_text(f"<b>Found {total} Files For Your Query {keyword}!!</b>")
        deleted = 0
        async with lock:
            try:
                for file in files:
        ...
        await query.answer('Succesfully Reset...')
        await query.message.edit_text("<b>Successfully Reset Group Setting...\n\nNow Send /details Again</b>", reply_markup=reply_markup)
        ...
        if settings is not None:
            ...
            await query.message.edit_text("<b>Somthing Went Wrong</b>")
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("Accept This Request", callback_data=f"accept#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("Reject This Request", callback_data=f"reject#{user_id}#{msg_id}")
        ]]
        try:
            ...
            await query.answer("You Are A Not Member Of This Channel, First Join", show_alert=True)
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Sorry Your Request Is Rejected</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>Hello {user.mention},\n\Sorry Your Request Is Rejected</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton("‼️ Not Available ‼️", callback_data=f"not_available#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("Tell Me Year/Language", callback_data=f"year#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("Upload In 1 Hour", callback_data=f"upload_in#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("☇ Upload ☇", callback_data=f"uploaded#{user_id}#{msg_id}")
        ]]
        try:
            st = await client.get_chat_member(chnl_id, userid)
            if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.answer("You Are Not Member Of This Channel, First Join", show_alert=True)
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Sorry Your Request Is Not Available</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>Hello {user.mention},\n\nSorry Your Request Is Not Found</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Your Request Is Uploaded</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>Hello {user.mention},\n\nYour Request Is Uploaded</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Your Request Is Already Available</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>Hello {user.mention},\n\nYoi Request Is Already Available</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("Upload In 1 Hour", callback_data=f"upload_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("View Status", url=f"{query.message.link}")
        ]]
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("Tell Me Year& Language", callback_data=f"yrs_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("View Status", url=f"{query.message.link}")
        ]]
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Sir Please Tell Me Language & Year Please Then I will Upload</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>Hello {user.mention},\n\nSir Please Tell Me Language & Year Please Then I Will Upload</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        if str(userid) in user_id:
            await query.answer("Sorry Your Query Is Rejected", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Sorry Your Request Is Rejected", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Your Request Will Be Uploaded Within 1 Hour", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Sir Please Tell Me Year & Language Please ThenI will Upload", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        btn = [[
        InlineKeyboardButton("Post Mode ➜", callback_data="update_post_mode"),
        InlineKeyboardButton(f"{'Single' if post_mode.get('singel_post_mode', True) else 'Multi'} Mode", callback_data="change_update_post_mode"),
    ],
    [
        InlineKeyboardButton("Upload Mode➜", callback_data="update_post_mode"),
        InlineKeyboardButton(f"{'All' if post_mode.get('all_files_post_mode', True) else 'New'} Files", callback_data="all_files_post_mode"),
    ]]
        reply_markup = InlineKeyboardMarkup(btn)
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    del_msg = f"\n\n<b> This Message Will Be  Aito Delete After <code>{get_readable_time(DELETE_TIME)}</code> To Avoid Copyright Issues</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
    ...
    if imdb:
        ...
        cap = f"<b>Here I Found For Your Search{search}</b>"
    del_msg = f"\n\n<b>This Message Will Be Deleted After<code>{get_readable_time(DELETE_TIME)}</code> To Avoid Copyright Issues</b>" if settings["auto_delete"] else ''
    CAP[key] = cap
...
async def advantage_spell_chok(message):
    ...
    if not movies:
        ...
        button = [[
            InlineKeyboardButton("Check Spelling In Google", url=f"https://www.google.com/search?q={google}")
        ]]
